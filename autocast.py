        ...
        for self.e in range(start, self.num_epochs):  # epoch
            for self.i, (source_input, reference_input) in enumerate(self.data_loader_train):  # batch
                ...
                with autocast():
                    ...
                    self.track("D_A")
                    d_loss_real = self.criterionGAN(out, True)  # 让D判断图片是否是真实参考图片（真实带妆图片）的loss
                    self.track("D_A_loss")
                    # Fake
                    # 利用生成网络生成fake_y合成妆容
                    ...
                    out = self.D_A(fake_A)
                    self.track("D_A_2")
                    d_loss_fake = self.criterionGAN(out, False)  # 让D判断图片是否是生成图片的loss
                    self.track("D_A_loss_2")

                    # Backward + Optimize
                    # 判别器网络反向传播，更新网络参数
                    d_loss = (d_loss_real.mean() + d_loss_fake.mean()) * 0.5  # 判别器A的两个loss（真实化妆图片和合成化妆图片）

                self.d_A_optimizer.zero_grad()
                scaler.scale(d_loss).backward()
                scaler.step(self.d_A_optimizer)
                scaler.update()
                ...
                # Logging
                self.loss = {}
                self.loss['D-A-loss_real'] = d_loss_real.mean().item()  # 判别器判断真实带妆图像能力

                # training D_B, D_B aims to distinguish class A 判断是否是“真source” x
                # Real
                with autocast():
                    out = self.D_B(image_s)
                    d_loss_real = self.criterionGAN(out, True)  # 看判别器判断是否是真实素颜图像的能力
                    # Fake 利用生成网络生成fake_x 卸妆图像
                    self.track("G-before")
                    fake_B = self.G(image_r, image_s, mask_r, mask_s, dist_r, dist_s)
                    self.track("G-2")
                    fake_B = Variable(fake_B.data).detach()  # 看判别器判断是否是生成的素颜图像的能力
                    out = self.D_B(fake_B)
                    d_loss_fake = self.criterionGAN(out, False)

                    # Backward + Optimize
                    d_loss = (d_loss_real.mean() + d_loss_fake.mean()) * 0.5  # 上面判别器B的两个Loss加起来（真实素颜图片和合成素颜图片）

                self.d_B_optimizer.zero_grad()
                scaler.scale(d_loss).backward()
                scaler.step(self.d_B_optimizer)
                scaler.update()

                # d_loss.backward(retain_graph=False)
                # self.d_B_optimizer.step()           #算梯度

                # Logging
                self.loss['D-B-loss_real'] = d_loss_real.mean().item()  # 判别器判断素颜图是否是真实图像

                # self.track("Discriminator backward")

                # ================== Train G ================== #       #训练生成器 G的第一个参数是素颜图提供人脸身份信息的，第二个参数是提供妆容信息的
                if (self.i + 1) % self.g_step == 0:
                    with autocast(True):
                        ...
                        idt_A = self.G(image_s, image_s, mask_s, mask_s, dist_s,
                                       dist_s)  # 用一个图片既当素颜图又当参考图出来的图应该是一样的，和它本身
                        idt_B = self.G(image_r, image_r, mask_r, mask_r, dist_r, dist_r)
                        #print("idt_A:",idt_A.shape)
                        ...
                        fake_A = self.G(image_s, image_r, mask_s, mask_r, dist_s, dist_r)  # 真正的计算两张不同的图的结果是否真实
                        pred_fake = self.D_A(fake_A)
                        g_A_loss_adv = self.criterionGAN(pred_fake, True)  # 生成对抗损失A

                        # GAN loss D_B(G_B(B))
                        fake_B = self.G(image_r, image_s, mask_r, mask_s, dist_r, dist_s)
                        pred_fake = self.D_B(fake_B)
                        g_B_loss_adv = self.criterionGAN(pred_fake, True)  # 生成对抗损失B
                        ...
                        rec_A = self.G(fake_A, image_s, mask_s, mask_s, dist_s,
                                       dist_s)  # 循环一致性损失，上面生成的化妆图和原素颜图再送进去应该生成卸妆图
                        rec_B = self.G(fake_B, image_r, mask_r, mask_r, dist_r, dist_r)

                        g_loss_rec_A = self.criterionL1(rec_A, image_s) * self.lambda_A  # 生成的应该是素颜图，所以和原素颜图算L1损失
                        g_loss_rec_B = self.criterionL1(rec_B, image_r) * self.lambda_B
                        # self.track("Generator recover")

                        self.track('vgg start')
                        # vgg loss  感知损失是判断身份信息的loss
                        # Perceptual loss
                        vgg_s = self.vgg(image_s)  # 素颜图和生成的图像身份特征一致    #提取原素颜图的人脸身份信息
                        vgg_s = Variable(vgg_s.data).detach()
                        vgg_fake_A = self.vgg(fake_A)  # 提取生成的化妆图的人脸身份信息
                        g_loss_A_vgg = self.criterionL2(vgg_fake_A, vgg_s) * self.lambda_A * self.lambda_vgg
                        # self.track("Generator vgg")

                        vgg_r = self.vgg(image_r)  # 参考图和生成的图像妆容一致
                        ...
                        # Combined loss 生成器的所有损失
                        g_loss = (
                                g_A_loss_adv + g_B_loss_adv + loss_rec + loss_idt + g_A_loss_his + g_B_loss_his).mean()
                    ...
                    self.loss['G-loss-org'] = g_loss_rec_A.mean().item()  # 能不能算回原图
                    self.loss['G-loss-ref'] = g_loss_rec_B.mean().item()  # 能不能算回原图
                    ...
                    self.loss['G-loss-vgg-rec'] = (g_loss_A_vgg + g_loss_B_vgg).mean().item()  # 感知损失
                    self.loss['G-loss-imgA-rec'] = g_loss_rec_A.mean().item()
                    self.loss['G-loss-vggA-rec'] = g_loss_A_vgg.mean().item()
