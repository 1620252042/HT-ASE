...
def main(): #--type blend --input data/crop/ --output data/crop/blend/ \--landmark_input data/crop/landmark.pk --keep_eye_mouth --include_forehead --adjust_color --adjust_lighting
    parser = argparse.ArgumentParser(description='To handle which dataset and how to handle it.')

    # Basic arguments
    parser.add_argument("--type", type=str, choices=['pcg', 'copy', 'filter', 'crop', 'blend', 'crop_and_blend', 'mask', 'landmark'], default="filter", help="how to handle the dataset") #--type blend
    parser.add_argument("--input", type=str, help="the root of the original dataset")   #--input data/crop/
    parser.add_argument("--output", type=str, help="the root to store/find the filtered images by landmark detection")  #--output data/crop/blend/
...
def blend(before_files, after_files, args):
    input = args.input      #--input data/crop/
    output = args.output    #--output data/crop/blend/

    if args.detector == "api":
        api_landmarks = pickle.load(open(args.landmark_input, 'rb'))    #--landmark_input data/crop/landmark.pk
