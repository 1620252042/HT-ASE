...
from facemorpher import locator, blender
from facemorpher.warper import *
from imutils import face_utils
from scipy.stats import multivariate_normal
import torch
from constants import *
from psgan.utils.api_util import FacePPAPI
from psgan.utils.geo_util import expandConvex
...
def faceMaskByHue(img, threshold=0.15):
    ...
    #print(img_hsv[:, :, 2])
    mask = np.logical_or(img_hsv[:, :, 0] < threshold,
                         img_hsv[:, :, 2] < 6)  # numpy中数组的矩阵的逻辑操作。and/or/not分别为与/或/非 x1与x2对应位置的或操作
    return mask
...
class FaceCropper():
    ...
    def warpFace(self, before_img, after_img,
        ...
        before_img = before_img.permute(1, 2, 0).cpu().numpy().copy().astype(np.uint8)
        after_img = after_img.permute(1, 2, 0).cpu().numpy().copy().astype(np.uint8)
        before_img = cv2.merge([before_img[:, :, 2], before_img[:, :, 1], before_img[:, :, 0]])
        after_img = cv2.merge([after_img[:, :, 2], after_img[:, :, 1], after_img[:, :, 0]])

        before_points = before_points.cpu().numpy().astype(np.int32)[0]
        after_points = after_points.cpu().numpy().astype(np.int32)[0]

        # before_img = before_img[:, :, [2, 1, 0]]
        # after_img = after_img[:, :, [2, 1, 0]]
        ...
        if additional_gaussian:
            ...
            x, y = np.mgrid[0:before_shape[0]:1, 0:before_shape[1]:1]  # w,h
        ...
        if use_tps:
            if before_shape[0] > after_shape[0] or before_shape[1] > after_shape[1]:
                expanded = np.zeros((max(before_shape[0], after_shape[0]), max(before_shape[1], after_shape[1]), 3),
                                    dtype=np.uint8)
                expanded[:after_shape[0], :after_shape[1], :] = after_img
            else:
                expanded = after_img
            after_transformed = warpImageTPS(after_points, result_points,
                                             cropImageByPointsConvex(expanded, after_points))
        ...
        poisson_face = cv2.seamlessClone(after_transformed, before_img, face_mask.astype(np.uint8) * 255,
                                         (x + w // 2, y + h // 2), clone_method)
        self.poisson_face = poisson_face

        # Logger.current_logger().report_image('p_transer', 'poisson_face', 1, image=poisson_face)
        ...
        if extra_blending_for_extreme:
            ...
            combined = (after_combined_mask * extra_blending_weight) * after_transformed.astype(float) + \
                       (after_combined_mask * (1 - extra_blending_weight) + (~after_combined_mask)) * result_img.astype(
                float)
        ...
        result_img= torch.from_numpy(result_img.copy())[:, :, [2, 1, 0]].cuda()
    ...
    def faceParts(self, result_img, result_points, landmark_type="stasm", use_circle=True, expand_ratio=1.0,
                  color=None):
        parts = []
        rects = []
    ...
    def generateTrainingData(self,
        ...
        parts, rects = self.faceParts(image, landmarks, landmark_type=landmark_type, use_circle=False, expand_ratio=1.0,
                                      color=bg_color)
        results = []
    ...
    def cutAlignedFace(self,
        ...
        for (name, (i, j)) in face_utils.FACIAL_LANDMARKS_IDXS.items():
            ...
            roi = face_aligned_image[center[0] - half_size:center[0] + half_size,
