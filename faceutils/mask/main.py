...
class FaceParser:
    ...
    def parse(self, image: Image):
        ...
        with torch.no_grad():
            ...
            #print(parsing.shape)
        parsing = torch.nn.functional.embedding(parsing, self.dic.unsqueeze(1))# shou ji mei dian le ,can shu fan le ??? sha yi si,di er ju?
        # 第一个参数是整数列表，第二个才是weight，奥奥 还是不对？
